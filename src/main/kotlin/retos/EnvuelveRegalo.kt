...
un algoritmo que le ayude en la tarea. A la máquina se le pasa un array con los regalos. Cada regalo es un string.
...
fun envuelveRagalo () {
    println("Ingrese un lista separada por espacios")
    val inputList = readLine()!!.split(" ").map { it }.toMutableList()

    inputList.forEach { palabra ->
        val sizePalabra = palabra.length + 4
        println("*".repeat(sizePalabra))
        print("* $palabra *")
        println()
        println("*".repeat(sizePalabra))
        println()
